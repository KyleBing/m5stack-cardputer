# Battery

Main menu key: `p`

Shows live charge level and roughly **1 hour** of battery history (including sleep-segment markers).

## Screenshots

<div class="shot-row">

![battery](/shots/app_battery.png)

</div>

## Shortcuts

This App’s footer is mainly legend text; there are no dedicated letter action keys.

| Legend | Meaning |
|--------|---------|
| 1h log | Historical samples |
| live | Live segment |
| sleep | During sleep |
| now | Current point |

Back: `ESC` / `GO`. Page keys do nothing when there is no extra page.

## Usage

1. Open to see current percentage and the curve.
2. History is sampled in the background; deep sleep and similar states are marked on the curve.
3. Useful for spotting runtime and sleep drain trends — not a precision fuel-gauge calibration tool.
